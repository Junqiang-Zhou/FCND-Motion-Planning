...
# prune paths using collinearity check
def point(p):
...
def prune_path(path):
    if path is not None:
        ...
        while idx < len(pruned_path)-2:
            p1 = point(pruned_path[idx-1])
            p2 = point(pruned_path[idx])
            p3 = point(pruned_path[idx+1])
            if collinearity_check(p1, p2, p3, epsilon=1e-3):
                pruned_path.pop(idx)
            else:
                idx += 1
    else:
